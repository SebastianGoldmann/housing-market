...
class Population:
    ...
    def transitions(self):
        # Initialize dictionaries to store the count of each person type and housing type
        starter_count = {
            HousingTypes.NOHOUSE: 0,
            HousingTypes.ENTRYHOME: 0,
            HousingTypes.LUXURY: 0
        }
        mover_count = {
            HousingTypes.NOHOUSE: 0,
            HousingTypes.ENTRYHOME: 0,
        ...
        # Loop through the population and increment the count for each person type and housing type
        for person in self.population:
            if person.personType == PersonTypes.STARTER:
                starter_count[person.housingType] += 1
            else: # person is a mover
                mover_count[person.housingType] += 1
        
        # Store the count for each person type and housing type in the appropriate list
        self.starterToOtherHouse.extend(starter_count.values())
        self.moverToOtherHouse.extend(mover_count.values())
    ...
    def simulate(self, tradingYears):
        ...
        if not self.population:
            ...
            for _ in tqdm(range(tradingYears)):
                ...
                self.trade()
                #calculate number of persons who moved houses
                self.transitions()
