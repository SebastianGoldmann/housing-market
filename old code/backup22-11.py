...
class Population:
    ...
    def plotPrices(self, plotLuxury:bool):
        averageLuxuryPrices = []
        averageEntryPrices = []
        # Get the average housing price for each Day
        for dailyHousePrice in self.luxuryHousePrices:
            if len(dailyHousePrice) == 0:
                averageLuxuryPrices.append(None)
            else:
                average = sum(dailyHousePrice)/len(dailyHousePrice)
                averageLuxuryPrices.append(average)

        for dailyHousePrice in self.entryHousePrices:
            if len(dailyHousePrice) == 0:
                averageEntryPrices.append(None)
            else:
                average = sum(dailyHousePrice)/len(dailyHousePrice)
                averageEntryPrices.append(average)


        # Plot the average housing price per Day
        if plotLuxury:
            plt.plot(range(len(averageLuxuryPrices)),averageLuxuryPrices, 'ro')
            plt.xlabel('Days')
            plt.ylabel('$')
            plt.ylim(bottom=0)
            plt.title('Average luxury housing Price')
            plt.show()
        else:
            plt.plot(range(len(averageEntryPrices)),averageEntryPrices, 'ro')
            plt.xlabel('Days')
            plt.ylabel('$')
            plt.ylim(bottom=0)
            plt.title('Average luxury housing Price')
...
def main():
    ...
    population.createMovers(numbOfMsovers=200, percentageInEntry=0.5, intialWealth=100_000)
    population.createStarters(numbOfStarters=200, intialWealth=600_000)
    population.simulate(tradingDays=200)
    population.plotPrices(plotLuxury=True)
    population.plotPrices(plotLuxury = False)
    '''
